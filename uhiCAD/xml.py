...
def add_ground_from_XYZ(district, terrain_df, zone_box, center_coordinates, buildings, kFactor=0.1, groundtype=31, detailedSimulation=False, ShortWaveReflectance=0.14):    
    #Ground unification for each building
    buildings['floor_union'] = buildings['floor'].apply(lambda x: unary_union(x))
    floor_geom = gpd.GeoDataFrame(geometry=buildings['floor_union'])
    #plot_polygon(floor_geom.geometry.unary_union)
    ...
    # Cretion of a geodataframe for triangles in zone_box
    gdf = gpd.GeoDataFrame(geometry=geometry_list, crs='EPSG:2056')
    gdf['gid'] = id_list
    
    # Creation of the ground geodataframe for the baseline
    ground_data_baseline = gdf.copy()
    
    columns = ['geometry', 'bid', 'egid', 'envelope_type']
    zone_geom = gpd.GeoDataFrame(columns=columns)
    for i in range(len(buildings)):
        # Selection of the ground geometry to intersect with triangles
        if buildings.at[i,'envelope_type'] == '2.5D':
            geom_ground = buildings.at[i,'geometry']
        elif buildings.at[i,'envelope_type'] == '3D':
            geom_ground = floor_geom.at[i,'geometry']
        #Save building ground geometry and egid   
        zone_geom.loc[len(zone_geom)] = {'geometry': geom_ground,'bid': buildings.at[i, 'bid'], 'egid': buildings.at[i, 'egid'], 'envelope_type': buildings.at[i, 'envelope_type']}
        zone_geom_qgis = zone_geom[['geometry', 'bid']].copy()
        
        for index, row in gdf.iterrows():
            triangle_geometry = row.geometry
            triangle_id = row['gid']
            if triangle_geometry.intersects(geom_ground):
                # Check if the triangle is already in the list
                if triangle_id in idintersection_list:
                    continue
                
                intersection_list.append(Polygon(triangle_geometry.exterior.coords))
                idintersection_list.append(triangle_id)
                indexes_to_drop.append(index)
              
    # Creation of a geodataframe for the triangles of the intersection
    gdf_intersection = gpd.GeoDataFrame(geometry=intersection_list, crs='EPSG:2056')
    gdf_intersection['gid'] = idintersection_list
    gdf.drop(indexes_to_drop, inplace=True) 
    
     
    return gdf, gdf_intersection, ground_data_baseline, floor_geom, zone_geom, zone_geom_qgis
